    ...
    public class SvgBuilder
    {
        ...
        public string Build(List<Shape> shapes, bool noise)
        {
            return
                ...
                + SVG_FILE_HEADER
                + SVG_TAG_HEADER
                + (BuildShapeLoop(shapes, noise))
                + SVG_TAG_FOOTER;
        }

        private StringBuilder BuildShapeLoop(List<Shape> shapes, bool noise)
        {
            StringBuilder BuildShapeText = new StringBuilder();

                for (int i = 0; i < shapes.Count; i++)
                {
                    BuildShapeText.Append(shapes[i].BuildSVGString(noise));
                }
            return BuildShapeText;
        }
    }
}
